...
CONSTRAINTS = {
    "spym": {"min": 10.0, "max": 80.0},
    "vea": {"min": 5.0, "max": 40.0},
    "agg": {"min": 10.0, "max": 80.0},
}

MODEL_VERSION = "saa_taa_v1"
ASSUMPTION_SET = "core3_hybrid_rules"

# ---------------------------------------------------------------------------
# Predefined portfolio configurations  (3 types × 2 risk levels = 6 total)
# Asset keys: us_eq, intl_eq, fixed, alts, cash
# ---------------------------------------------------------------------------
...
PORTFOLIOS: dict[str, dict] = {
    "core_growth": {
        "label": "Core",
        "weights": {"us_eq": 55.0, "intl_eq": 20.0, "fixed": 15.0, "alts": 5.0, "cash": 5.0},
        "tickers": {"us_eq": "SPY", "intl_eq": "VEA", "fixed": "AGG", "alts": "QAI", "cash": "BIL"},
    },
    "core_conservative": {
        "label": "Core",
        "weights": {"us_eq": 25.0, "intl_eq": 10.0, "fixed": 45.0, "alts": 5.0, "cash": 15.0},
        "tickers": {"us_eq": "SPY", "intl_eq": "VEA", "fixed": "AGG", "alts": "QAI", "cash": "BIL"},
    },
    "socially_responsible_growth": {
        "label": "Socially Responsible",
        "weights": {"us_eq": 55.0, "intl_eq": 20.0, "fixed": 15.0, "alts": 5.0, "cash": 5.0},
        "tickers": {"us_eq": "ESGU", "intl_eq": "ESGD", "fixed": "EAGG", "alts": "QAI", "cash": "BIL"},
    },
    "socially_responsible_conservative": {
        "label": "Socially Responsible",
        "weights": {"us_eq": 25.0, "intl_eq": 10.0, "fixed": 45.0, "alts": 5.0, "cash": 15.0},
        "tickers": {"us_eq": "ESGU", "intl_eq": "ESGD", "fixed": "EAGG", "alts": "QAI", "cash": "BIL"},
    },
    "direct_indexing_growth": {
        "label": "Direct Indexing",
        "weights": {"us_eq": 55.0, "intl_eq": 20.0, "fixed": 15.0, "alts": 5.0, "cash": 5.0},
        "tickers": {"us_eq": "VTI", "intl_eq": "VXUS", "fixed": "BND", "alts": "QAI", "cash": "SGOV"},
    },
    "direct_indexing_conservative": {
        "label": "Direct Indexing",
        "weights": {"us_eq": 25.0, "intl_eq": 10.0, "fixed": 45.0, "alts": 5.0, "cash": 15.0},
        "tickers": {"us_eq": "VTI", "intl_eq": "VXUS", "fixed": "BND", "alts": "QAI", "cash": "SGOV"},
    },
...
PORTFOLIO_BASE_NAMES = ("core", "socially_responsible", "direct_indexing")


def _risk_profile_display_name(risk: int) -> str:
    if risk <= 2:
...
def _blend_portfolio_weights(cons: dict, grow: dict, risk: int) -> dict:
    """Linear blend: risk 1 → conservative targets, risk 10 → growth targets."""
    t = (risk - 1) / 9.0
    keys = cons["weights"].keys()
    blended = {
        k: cons["weights"][k] * (1.0 - t) + grow["weights"][k] * t for k in keys
    }
    rounded = {k: round(blended[k], 2) for k in keys}
    total = sum(rounded.values())
    diff = round(100.0 - total, 2)
    if keys and abs(diff) > 0.001:
        first = next(iter(rounded))
        rounded[first] = round(rounded[first] + diff, 2)
    return rounded


def _parse_optional_risk(qs: dict) -> int | None:
    raw = qs.get("risk", [None])[0]
    if raw is None:
        return None
    try:
...
def _resolve_dashboard_portfolio(portfolio_type: str, qs: dict) -> tuple[str, int] | None:
    """Returns (base_name, risk_1_10) for dashboard requests, or None if unknown."""
    p = portfolio_type.strip().lower()
    risk_opt = _parse_optional_risk(qs)

    for base in PORTFOLIO_BASE_NAMES:
        if p == base:
            return (base, risk_opt if risk_opt is not None else 5)
        if p == f"{base}_growth":
            return (base, risk_opt if risk_opt is not None else 10)
        if p == f"{base}_conservative":
            return (base, risk_opt if risk_opt is not None else 1)
    return None
...
def compute_saa(profile_inputs: dict) -> dict:
    ...
    total_equity = 30.0 + (40.0 * risk_n) + (20.0 * horizon_n) - (10.0 * liquidity_n) + (5.0 * drawdown_n)
    total_equity = _clamp(total_equity, 15.0, 90.0)

    intl_share = _clamp(0.20 + (0.15 * risk_n) + (0.10 * horizon_n), 0.15, 0.45)
    vea = total_equity * intl_share
    spym = total_equity - vea
    agg = 100.0 - spym - vea

    strategic = {"spym": spym, "vea": vea, "agg": agg}
...
def compute_taa_overlays(market_inputs: dict, tactical_intensity: float) -> tuple[dict, dict]:
    ...
    raw_signals = {
        "spym": (2.0 * growth) - (1.0 * rates) - (2.0 * volatility),
        "vea": (1.5 * growth) + (0.5 * inflation) - (1.5 * volatility),
        "agg": (-1.5 * growth) - (2.0 * rates) + (1.5 * volatility),
    }
...
def infer_profile_from_final_allocation(final_weights: dict) -> dict:
    total_equity = final_weights["spym"] + final_weights["vea"]
    risk_score = 1.0 + (total_equity / 90.0) * 9.0
    horizon = 1.0 + (total_equity / 90.0) * 29.0
    liquidity_need = 1.0 + (final_weights["agg"] / 80.0) * 9.0
    drawdown_tolerance = 1.0 + (total_equity / 90.0) * 9.0
...
def app(environ, start_response):
    ...
    portfolio_type_param = qs.get("portfolio_type", [None])[0]
    if portfolio_type_param is not None:
        portfolio_type_param = portfolio_type_param.strip().lower()
        resolved = _resolve_dashboard_portfolio(portfolio_type_param, qs)
        if resolved is None:
            valid = [
                *PORTFOLIO_BASE_NAMES,
                *[f"{b}_growth" for b in PORTFOLIO_BASE_NAMES],
                *[f"{b}_conservative" for b in PORTFOLIO_BASE_NAMES],
            ]
            return _json_response(
                "400 Bad Request",
                {"error": f"Unknown portfolio_type. Valid options: {valid}"},
                start_response,
            )
        base, risk_score = resolved
        cons = PORTFOLIOS[f"{base}_conservative"]
        grow = PORTFOLIOS[f"{base}_growth"]
        weights = _blend_portfolio_weights(cons, grow, risk_score)
        response = {
            "portfolio_key": f"{base}_r{risk_score}",
            "portfolio_label": cons["label"],
            "risk_score": risk_score,
            "risk_profile": _risk_profile_display_name(risk_score),
            "weights": weights,
            "tickers": grow["tickers"],
            "asset_labels": PORTFOLIO_ASSET_LABELS,
            "calc_timestamp": datetime.now(timezone.utc).isoformat(),
        }
        return _json_response("200 OK", response, start_response)

    # --- Legacy SAA/TAA endpoint ---
